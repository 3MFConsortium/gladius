    ...
    bool NodeBase::updateTypes(nodes::Model & model)
    {
        ...
        for (auto & parameter : m_parameter)
        {
            ...
            if (source.has_value())
            {
                ...
                {
                    // Ignore and continue; type inference will fail gracefully
                }
        ...
        auto ruleIter = std::find_if(
          m_typeRules.begin(),
          m_typeRules.end(),
          [&inputTypeMap](const TypeRule & rule)
          {
              if (rule.input.size() !=
                  inputTypeMap.size()) // this happens for nodes that can use scalar or vector types
              {

...
namespace gladius::io
{
    ...
    void Writer3mf::updateModel(Document const & doc)
    {
        ...
        for (auto & [name, model] : functions)
        {
            ...
            // Detect unsupported FunctionGradient and NormalizeDistanceField nodes and skip
            // serialization for such models until 3MF specification & lib3mf support is available.
            ...
            if (detector.found)
            {
                m_logger->addEvent(
                  {fmt::format(
                     "Skipping function '{}' during 3MF export: contains FunctionGradient or "
                     "NormalizeDistanceField node(s) which are not yet supported in 3MF "
                     "serialization.",
                     name),
                   events::Severity::Warning});

...
namespace gladius::io
{
                                                     ...
                                                     Lib3MF::CImplicitFunction & function)
    {
        ...
        auto resourceIterator = const_cast<Lib3MF::CModel &>(model).GetFunctions();

        // Iterate through resources
        while (resourceIterator->MoveNext())
        {
            auto resource = resourceIterator->GetCurrentFunction();
            if (!resource)
            {
                continue;
            }
            ...
            if (function.GetUniqueResourceID() == existingFunction->GetUniqueResourceID())
            {
                continue;
            }

...
namespace gladius
{
    namespace mcp::tools
    {
        ...
        FunctionOperationsTool::createSDFFunction(const std::string & name,
        ...
        std::pair<bool, uint32_t>
        FunctionOperationsTool::createCSGOperation(const std::string & name,
                                                   ...
                                                   float blendRadius)
        {
            ...
            if (operation == "union")
            {
                ...
                for (size_t i = 1; i < operands.size(); ++i)
                {
                    if (smooth)
                    {
                        expression = "smoothMin(" + expression + ", " + operands[i] +
                                     "_distance, " + std::to_string(blendRadius) + ")";
                    }
                    else
                    {
            ...
            else if (operation == "difference")
            {
                ...
                for (size_t i = 1; i < operands.size(); ++i)
                {
                    if (smooth)
                    {
                        expression = "smoothMax(" + expression + ", -" + operands[i] +
                                     "_distance, " + std::to_string(blendRadius) + ")";
                    }
                    else
                    {
        ...
        bool
        FunctionOperationsTool::applyTransformToFunction(const std::string & functionName,
                                                         ...
                                                         const std::array<float, 3> & scale)
        {
            ...
            {
                expression += " / vec3(" + std::to_string(scale[0]) + ", " +

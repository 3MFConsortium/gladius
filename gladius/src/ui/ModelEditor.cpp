...
namespace gladius::ui
{
    ...
    void ModelEditor::copySelectionToClipboard()
    {
        ...
        for (auto const & [origId, clonedNode] : cloneMap)
        {
            ...
            for (auto const & [paramName, param] : origNode->constParameter())
            {
                ...
                nodes::Port * clonedSrcPort = cloneMap[srcNodeId]->findOutputPort(srcPort->getShortName());
    ...
    else
    {
        ...
        for (auto const & [id, nodePtr] : *m_clipboardModel)
        {
            ...
            ed::SetNodePosition(inserted->getId(), ImVec2(inserted->screenPos().x, inserted->screenPos().y));
        }
        ...
        for (auto const & [origName, newNode] : pastedMap)
        {
            ...
            for (auto const & [paramName, param] : origNode->constParameter())
            {
                ...
                if (pastedSrcIt == pastedMap.end())
                    continue;
                nodes::Port * newSrcPort = pastedSrcIt->second->findOutputPort(origSrcPort->getShortName());
